...
class WebhookHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        # Auto-formato simplificado y legible para alertas de Telegram
        if "api.telegram.org" in self.url:
            try:
                raw_json = json.loads(message)
                src = raw_json.get("src_host", "Desconocido")
                dst_port = raw_json.get("dst_port", "Desconocido")
                local_time = raw_json.get("local_time", "").split(".")[0]
                services = {
                    21: "FTP (Transferencia de archivos)",
                    22: "SSH (Consola remota)",
                    2222: "SSH Trampa (Honeypot)",
                    80: "Web / NAS Falso",
                    445: "SMB (Carpetas Windows)",
                    3306: "MySQL (Base de datos)",
                    6379: "Redis (Base de datos)"
                }
                srv = services.get(dst_port, f"Puerto {dst_port}")
                logdata = raw_json.get("logdata", {})
                lines = [
                    "🚨 *[HONEYPOT ALERT] Intrusión Detectada*",
                    "━━━━━━━━━━━━━━━━━━━━",
                    f"🎯 *Servicio:* `{srv}`",
                    f"🌐 *IP Atacante:* `{src}`",
                    f"🕒 *Hora:* `{local_time}`"
                ...
                if isinstance(logdata, dict):
                    if "USERNAME" in logdata or "PASSWORD" in logdata:
                        u = logdata.get("USERNAME", "-")
                        p = logdata.get("PASSWORD", "-")
                        lines.append(f"👤 *Usuario:* `{u}`")
                        lines.append(f"🔑 *Password:* `{p}`")
                    elif "PATH" in logdata:
                        path = logdata.get("PATH", "/")
                        skin = logdata.get("SKIN", "-")
                        lines.append(f"📄 *Ruta:* `{path}` (Tema: `{skin}`)")
                    elif "COMMAND" in logdata:
                        cmd = logdata.get("COMMAND", "-")
